...
function renderRecaptcha(siteKey) {
  ...
  return `
    <div class="recaptcha-block">
      <div class="g-recaptcha" data-sitekey="${escapeHtml(siteKey)}"></div>
      <p class="form-note">This security check helps reduce spam from automated submissions.</p>
    </div>
  `;
}
...
function renderPage({ path, title, description, hero, content, flash = "", extraHead = "" }) {
  const navLinks = nav
    .map((item) => {
...
function renderSurfkitPage(status = "", options = {}) {
  ...
  return renderPage({
    ...
    extraHead: options.recaptchaSiteKey ? '<script src="https://www.google.com/recaptcha/api.js" async defer></script>' : "",
  });
}
