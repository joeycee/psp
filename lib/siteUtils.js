...
async function sendContactEmail({ name, companyName, email, message, enquiryType, sourcePage, env = process.env, fetchImpl = global.fetch }) {
  ...
  if (!config.enabled || !fetchImpl) {
    console.log("[contact-email] Email sending unavailable: missing SMTP2GO config");
    return { ok: false, reason: "unavailable" };
  }

  console.log("[contact-email] Sending via SMTP2GO", {
    to: config.to,
    from: config.from,
  ...
  if (!response.ok) {
    console.log("[contact-email] SMTP2GO HTTP error", {
  ...
  if (result.data?.failed > 0 || result.data?.succeeded === 0) {
    console.log("[contact-email] SMTP2GO reported failure", {
  ...
  console.log("[contact-email] SMTP2GO send success", {
    succeeded: result.data?.succeeded,
    failed: result.data?.failed,
  });
